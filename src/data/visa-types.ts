...
const sourceSets: Record<string, SourceLink[]> = {
  ...
  germany: [
    {
      label: "German missions visa guidance",
      url: "https://www.auswaertiges-amt.de/en/visa-service",
    },
  ],
  uae: [{ label: "ICP visa services", url: "https://icp.gov.ae/en/services/issue-entry-permit/" }],
  india: [{ label: "Indian Visa Online", url: "https://indianvisaonline.gov.in/" }],
};
...
const baseTemplates: Record<VisaCategory, Omit<VisaType, "countryCode"> & { titleSuffix: string }> =
  {
    ...
    work: {
      ...
      bestFor: [
        ...
        "Assigned international transfers",
      ],
      eligibilitySummary:
...
const overrides: Record<string, Partial<Record<VisaCategory, Partial<VisaType>>>> = {
  usa: {
    tourist: {
      ...
      validityMonths: 120,
      bestFor: [
        "Long-validity visitor travel",
        "Family visits",
        "Tourism with fixed interview planning",
      ],
      eligibilitySummary:
